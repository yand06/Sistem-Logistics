    ...
    <>
      ...
      <Section>
        <div className="flex justify-end -mb-2">
          <Btn data-testid="new-invoice-btn" onClick={() => setOpen(true)}>
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                label: "PPN",
                align: "right",
                render: (r) => formatCurrency(r.ppn_amount, r.currency),
              },
              {
                key: "total",
                label: "Total",
              ...
              },
              {
                ...
                render: (r) => (
                  <div className="flex gap-2 justify-end">
                    <Btn
                      variant="outline"
                      data-testid={`xml-${r.id}`}
                      onClick={() => exportXML(r.id, r.invoice_no)}
                    >
                      <FileCode2 className="h-3.5 w-3.5" /> Coretax XML
                    </Btn>
                    {r.status !== "paid" && (
                      <Btn
                        data-testid={`paid-${r.id}`}
...
function NewInvoice({ open, onClose, customers, jos }: any) {
  ...
  function updateLine(i: number, p: any) {
    setF((s: any) => ({
      ...s,
      lines: s.lines.map((l: any, idx: number) =>
        idx === i ? { ...l, ...p } : l,
      ),
    }));
  }
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3 border-t border-slate-300 pt-3">
          <Field label="PPN %">
            <input
              type="number"
              className={inputCls}
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Btn data-testid="inv-submit" onClick={() => mut.mutate(f)}>
            Issue Invoice
          </Btn>
        </div>
      </div>
