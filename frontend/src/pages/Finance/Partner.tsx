...
export default function PartnerPage() {
  ...
  const { data: partners = [] } = useQuery({ queryKey: ["partners"], queryFn: () => api.get("/partners") });
  const [f, setF] = useState<any>({ name: "", type: "vendor", contact_person: "", phone: "", email: "", bank_name: "", bank_account_no: "", bank_account_holder: "", notes: "" });
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Btn data-testid="new-partner-btn" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> New Partner</Btn>}
      />
      <Section>
        <Card>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New Partner" maxWidth="max-w-2xl">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Type">
            <select className={inputCls} value={f.type} onChange={(e) => setF({ ...f, type: e.target.value })}>
              {["vendor", "agent", "trucking", "shipping_line"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </Field>
          ...
          <Field label="Bank Account No."><input className={inputCls} value={f.bank_account_no} onChange={(e) => setF({ ...f, bank_account_no: e.target.value })} /></Field>
          <Field label="Account Holder"><input className={inputCls} value={f.bank_account_holder} onChange={(e) => setF({ ...f, bank_account_holder: e.target.value })} /></Field>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Btn data-testid="partner-submit" onClick={() => mut.mutate(f)}>Save</Btn>
        </div>
      </Modal>
    </>
  );
