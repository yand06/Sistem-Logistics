...
export default function JobOrder() {
  ...
  const stageMut = useMutation({
    mutationFn: ({ id, stage }: any) =>
      api.post(`/job-orders/${id}/shipment-status`, { stage }),
    onSuccess: () => {
  ...
  const cols: any[] = [
    ...
    { key: "eta", label: "ETA" },
    {
      key: "status",
      label: "Status",
      render: (r: any) => <StatusBadge status={r.status || "booking"} />,
    },
  ];
  if (isFinance) {
    cols.push(
      {
        ...
        label: "Buy",
        align: "right",
        render: (r: any) => r.buy_rate?.toLocaleString() || "—",
      },
      {
        key: "sell_rate",
        label: "Sell",
        align: "right",
        render: (r: any) => r.sell_rate?.toLocaleString() || "—",
      },
      {
        key: "margin",
        label: "Margin",
        align: "right",
        render: (r: any) =>
          r.sell_rate && r.buy_rate ? (
            <span className="text-emerald-700 font-medium">
              {(r.sell_rate - r.buy_rate).toLocaleString()}
            </span>
          ) : (
            "—"
          ),
  ...
  cols.push({
    ...
    render: (r: any) => (
      <div className="flex gap-2 justify-end">
        <Btn variant="outline" onClick={() => setSelected(r)}>
          Open
        </Btn>
      </div>
    ),
  ...
  return (
    <>
      ...
      <Section>
        ...
        <Card>
          ...
          <Table
            ...
            columns={cols}
            rows={jos}
            empty="No job orders yet. Execute a quotation to enable creation."
          />
        </Card>
      </Section>
      ...
      <NewJOForm
        ...
        quotations={quots.filter((q: any) => q.status === "executed")}
      />
      {selected && (
        <JODetail
          jo={selected}
    ...
    >
      <div className="space-y-4">
        <Field label="Executed Quotation">
          <select
            data-testid="jo-quotation"
            value={form.quotation_id}
            onChange={(e) => selectQ(e.target.value)}
            className={inputCls}
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="BL No.">
            <input
              className={inputCls}
              value={form.bl_no}
              onChange={(e) => setForm({ ...form, bl_no: e.target.value })}
            />
          </Field>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Btn data-testid="jo-submit" onClick={() => mut.mutate(form)}>
            Create Job Order
          </Btn>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <div className="text-slate-900">
              {customers.find((c: any) => c.id === jo.customer_id)?.name || "—"}
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-widest text-slate-400">
          ...
          <div>
            ...
            <div className="text-slate-900">{jo.vessel || "—"}</div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-widest text-slate-400">
              ETA
            </div>
            <div className="text-slate-900">{jo.eta || "—"}</div>
          </div>
        </div>
        ...
        {isFinance ? (
          <div className="border-t border-slate-300 pt-4">
            <div className="text-[11px] font-semibold uppercase tracking-widest text-red-600 mb-3">
              Finance Section (locked to Finance/Admin)
            </div>
              ...
              {[
                ...
                <Field key={k} label={k.replace(/_/g, " ")}>
                  <input
                    ...
                    value={finance[k]}
                    onChange={(e) =>
                      setFinance({ ...finance, [k]: +e.target.value })
                    }
                  />
                </Field>
              ))}
            ...
            <Field label="Finance Notes">
              <textarea
                rows={2}
                className={inputCls}
                value={finance.finance_notes}
                onChange={(e) =>
                  setFinance({ ...finance, finance_notes: e.target.value })
                }
              />
            </Field>
            <div className="flex justify-end mt-3">
              <Btn
                data-testid="jo-save-finance"
                onClick={() => patchMut.mutate(finance)}
              >
                Save Finance Data
              </Btn>
            </div>
          ...
          <div className="border-t border-slate-300 pt-4">
            <div className="text-[11px] uppercase tracking-widest text-slate-400">
              Finance section is restricted to Finance/Admin role.
            </div>
          </div>
        )}
      </div>
