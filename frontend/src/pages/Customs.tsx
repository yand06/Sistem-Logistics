...
export default function Customs() {
  ...
  const { data: docs = [] } = useQuery({ queryKey: ["customs-docs"], queryFn: () => api.get("/customs-docs") });
  const { data: jos = [] } = useQuery({ queryKey: ["job-orders"], queryFn: () => api.get("/job-orders") });
  const [f, setF] = useState<any>({ job_order_id: "", doc_number: "", doc_type: "PIB", status: "in_progress", notes: "" });
  ...
  const patchMut = useMutation({
    mutationFn: ({ id, status }: any) => api.patch(`/customs-docs/${id}`, { status }),
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Btn data-testid="new-customs-btn" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> New Customs Doc</Btn>}
      />
      <Section>
        <Card>
          ...
          <Table
            ...
            columns={[
              ...
              { key: "status", label: "Status", render: (r) => <StatusBadge status={r.status} /> },
              {
                ...
                align: "right",
                render: (r) => (
                  <select className={`${inputCls} inline-block w-40`} data-testid={`status-${r.id}`} value={r.status} onChange={(e) => patchMut.mutate({ id: r.id, status: e.target.value })}>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New Customs Doc">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <Field label="Doc Number"><input className={inputCls} value={f.doc_number} onChange={(e) => setF({ ...f, doc_number: e.target.value })} /></Field>
          <Field label="Doc Type">
            <select className={inputCls} value={f.doc_type} onChange={(e) => setF({ ...f, doc_type: e.target.value })}>
              {["PIB", "PEB", "PPFTZ", "Other"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </Field>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Btn data-testid="customs-submit" onClick={() => mut.mutate(f)}>Save</Btn>
        </div>
      </Modal>
    </>
  );
